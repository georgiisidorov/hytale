...
import { creditWallet, type WalletCurrency } from '@/lib/player-wallet';
...
export async function POST(req: Request) {
	try {
		...
		const currency = parseCurrency(body.currency) ?? 'coins';
		const amount = Number(body.amount);
		if (!Number.isFinite(amount) || amount <= 0) {
			return NextResponse.json({ ok: false, error: 'amount > 0' }, { status: 400 });
		}

		const reason = typeof body.reason === 'string' && body.reason.trim() ? body.reason.trim() : 'wheel_reward';
		const result = await creditWallet(getPool(), playerUuid, currency, amount, reason);
		if (!result.ok) {
