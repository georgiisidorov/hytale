// Patched: VIP+ can create markers, only OP can access config
...
public final class PermissionsUtil
{
    ...
    /** Permission to create map markers (when server allows). Else rank vip / admin / op. */
    private static final String MARKER_CREATE_PERMISSION = "bettermap.marker.create";
    ...
    /** True if player has rank vip, admin or op (or permission). Used for native map "Place new marker". */
    public static boolean canCreateMarkers(@Nonnull final Player player) {
        final PermissionsModule perms = PermissionsModule.get();
        if (perms == null) {
            return false;
        }
        final UUID uuid = ((CommandSender)player).getUuid();
        if (perms.hasPermission(uuid, MARKER_CREATE_PERMISSION)) {
            return true;
        }
        final Set<String> groups = perms.getGroupsForUser(uuid);
        if (groups == null) {
        ...
        return groups.contains("op") || groups.contains("admin") || groups.contains("vip");
    }
    ...
    /** Only OP can access config menu. */
